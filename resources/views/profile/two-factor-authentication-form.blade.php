    ...
    <x-slot name="content">
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                ...
                @if ($showingConfirmation)
                    <div class="mt-4">
                        <x-elements.label for="code" value="{{ __('Code') }}" />

                        <x-elements.input id="code" type="text" name="code" class="block mt-1 w-1/2" inputmode="numeric" autofocus autocomplete="one-time-code"
                            wire:model="code"
                            wire:keydown.enter="confirmTwoFactorAuthentication" />

                        <x-elements.input-error for="code" class="mt-2" />
                    </div>
                @endif
            @endif
        ...
        <div class="mt-5">
            @if (! $this->enabled)
                <x-sections.confirms-password wire:then="enableTwoFactorAuthentication">
                    <x-elements.button type="button" wire:loading.attr="disabled">
                        {{ __('Enable') }}
                    </x-elements.button>
            ...
            @else
                ...
                @if ($showingConfirmation)
                    <x-sections.confirms-password wire:then="disableTwoFactorAuthentication">
                        <x-elements.secondary-button wire:loading.attr="disabled">
                            {{ __('Cancel') }}
                        </x-elements.secondary-button>
                    </x-sections.confirms-password>
                @else
                    <x-sections.confirms-password wire:then="disableTwoFactorAuthentication">
                        <x-elements.danger-button wire:loading.attr="disabled">
                            {{ __('Disable') }}
