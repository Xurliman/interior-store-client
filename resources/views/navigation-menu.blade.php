            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                    <div class="ms-3 relative">
                        <x-elements.dropdown align="right" width="60">
                            <x-slot name="trigger">
                            ...
                            <x-slot name="content">
                                <div class="w-60">
                                    ...
                                    <!-- Team Settings -->
                                    <x-elements.dropdown-link href="{{ route('teams.show', Auth::user()->currentTeam->id) }}">
                                        {{ __('Team Settings') }}
                                    </x-elements.dropdown-link>

                                    @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                                        <x-elements.dropdown-link href="{{ route('teams.create') }}">
                                            {{ __('Create New Team') }}
                                        </x-elements.dropdown-link>
                                    @endcan
                                    ...
                                    @if (Auth::user()->allTeams()->count() > 1)
                                        ...
                                        @foreach (Auth::user()->allTeams() as $team)
                                            <x-sections.switchable-team :team="$team" />
                                        @endforeach
                                    @endif
                                </div>
                            </x-slot>
